import OrbitControls from './_OrbitControl';
import {MeshLine, MeshLineMaterial} from './_MeshLines';
import * as THREE from 'three';
import {points as globepoints} from './_Points';
...
var props = {
	...
	globeRadius: 300, // Radius of the globe (used for many calculations)
	dotsAmount: 20, // Amount of dots to generate and animate randomly across the lines
	startingCountry: 'uae', // The key of the country to rotate the camera to during the introduction animation (and which country to start the cycle at)
	colours: {
		// Cache the colours
...
export function getData() {
	...
	request.open('GET', '/en/locations.json', true);
...
function setupScene() {
	...
	var canvasResizeBehaviour = function() {
		...
		container.width = canvas.innerWidth;
		//container.height = window.innerHeight;
		container.style.width = canvas.innerWidth + 'px';
...
function addGlobeDots() {
	...
	for (var i = 0; i < globepoints.length; i++) {
		addDot(globepoints[i].x, globepoints[i].y);
	}
...
function createListElements() {
	...
	var pushObject = function(coordinates, target) {
		...
		// Commented to remove country label - requested by the client
		element.innerHTML = '<a href="'+targetCountry.url+'"><span class="text">'+ targetCountry.country +'</span></a>';
...
export function showCountry(key) {
	// var key = Object.keys(data.countries)[animations.countries.index];
	let inx = Object.keys(data.countries).findIndex(x=> x == key);
	if(inx){
		animations.countries.index = inx;
	}
	else{
		console.log('country not found on the map');
	}
	
	var key = Object.keys(data.countries)[inx];
	changeCountry(key, false);
